...
workflow with a Linux CLI, Linux paths, and login inside that distribution.
Do not mix Windows executables with WSL paths or assume the desktop Cursor
application installs the headless CLI.
...
Resolve the CLI to an executable path using the platform reference. If `agent`
is unavailable, try `cursor-agent`; check user-local install locations and a fresh
login shell before declaring it missing. Verify `--help` identifies Cursor CLI
and supports the flags below, since another application may own `agent`. Use the
resolved executable consistently for preflight, launch, and resume. If neither
exists, direct the user to the
...
The following examples work in **Bash and zsh**. Set `CURSOR_COMMAND` using the
POSIX reference first. Select the intended workspace explicitly.
...
"$CURSOR_COMMAND" --workspace "$WORKSPACE" \
...
Keep its handle (or the child PID and original shell above) and logs. Give runs time to
finish; poll at sensible intervals while doing independent work. A trailing `&`
is safe only when that shell remains alive; otherwise run the
command in the foreground through the managed terminal and retain its session
handle. A PID cannot be `wait`ed by a new shell. Do not launch a
duplicate because JSON output is still empty: it arrives at completion.

In the **same shell** that launched the Bash process:

```bash
if wait "$CURSOR_PID"; then EXIT_CODE=0; else EXIT_CODE=$?; fi
...
`subtype: "success"`, and `is_error: false`. Require a nonempty string
`session_id`, read `result`, and retain that ID.
...
"$CURSOR_COMMAND" --workspace "$WORKSPACE" --resume "$SESSION_ID" \
  --model cursor-grok-4.6-high --yolo --sandbox disabled --trust \
